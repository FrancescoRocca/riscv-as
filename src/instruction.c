...
static int is_pseudo_instr(const char *name) {
	...
	return strcmp(name, "li") == 0 || strcmp(name, "mv") == 0 || strcmp(name, "neg") == 0 || strcmp(name, "nop") == 0 ||
		   strcmp(name, "not") == 0 || strcmp(name, "seqz") == 0 || strcmp(name, "snez") == 0;
}

void expand_pseudo_instr(const instruction_s *instr, const char *lineBuf, const instruction_s **out_instructions,
...
static int32_t encode(const instruction_s *instr, char *lineBuf, char *name) {
	...
	switch (instr->type) {
		...
		case I_TYPE:
			...
			res =
				imm << 20 | (get_register(rs1) << 15) | (instr->funct3 << 12) | (get_register(rd) << 7) | instr->opcode;
			log_msg(LOG_DEBUG, "Write: %08x", res);
			break;
		case S_TYPE:
			...
			res = (GET_IMM_5_11(imm) << 25) | (get_register(rs2) << 20) | (get_register(rs1) << 15) |
				  (instr->funct3 << 12) | (GET_IMM_0_4(imm) << 7) | instr->opcode;
			log_msg(LOG_DEBUG, "Write: %08x", res);
			break;
		case B_TYPE:
